...
##1.Count number of missing values
  ...
  # View(missData2)
  # missingVar<-which(colMeans(is.na(df_data)) >= 0.5) #View removed variables
  # View(missingVar)
...
##4: Re-encode values equal labeled as "Don't know/Not sure" as NA. Re-encode values equal to None or never as 0 
  ...
  selDataset$CHILDREN[selDataset$CHILDREN == 88] <-0
...
##6. Factorize variables that are categorical using VAR# from spreadsheet
  ...
  sum(is.na(continuous))  #check that  # of NA values for continuous data =0
  ...
  categorical<-subset(categorical,select = -c(SAFETIME,CTELNUM1,CELLFON5))
